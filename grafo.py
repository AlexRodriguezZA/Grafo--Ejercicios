...
class Grafo(object):
    ...
    def MadresDeDioses(self,ver_origen):    
        while(ver_origen < self.inicio.tamanio()):
            vertice = self.inicio.obtener_elemento(ver_origen)
            if(not vertice['visitado']):
                vertice['visitado'] = True
                posicionDios = self.buscar_vertice(vertice['info'])
                dios = self.inicio.obtener_elemento(posicionDios);
                for i in range(dios["aristas"].tamanio()):
                    relacion = dios["aristas"].obtener_elemento(i)
                    if (len(relacion['data']['relacion']) == 2):
                        if (relacion['data']['relacion'][0] == "madre"):
                            print("Dios: ",relacion["destino"], " Madre: ",vertice['info'])
                aristas = 0
                while(aristas < vertice['aristas'].tamanio()):
                    arista = vertice['aristas'].obtener_elemento(aristas)
                    pos_vertice = self.buscar_vertice(arista['destino'])
                    nuevo_vertice = self.inicio.obtener_elemento(pos_vertice)
                    if(not nuevo_vertice['visitado']):
                        self.MadresDeDioses(pos_vertice)
                    aristas += 1
            ver_origen += 1
# grafo = Grafo()
